...
:loop
    ...
    for /f "usebackq tokens=2" %%A in (`adb %DEVICE_OPT% shell cat /proc/meminfo 2^>NUL ^| findstr /B /C:"SwapTotal"`) do set "SWAPTOTAL=%%A"
    for /f "usebackq tokens=2" %%A in (`adb %DEVICE_OPT% shell cat /proc/meminfo 2^>NUL ^| findstr /B /C:"SwapFree"`) do set "SWAPFREE=%%A"

    for /f "usebackq tokens=2,4" %%A in (`adb %DEVICE_OPT% shell df -k /data 2^>NUL ^| findstr /R /C:" /data$"`) do set "DATA_AVAIL=%%B"
    for /f "usebackq tokens=2,4" %%A in (`adb %DEVICE_OPT% shell df -k /storage/emulated 2^>NUL ^| findstr /R /C:" /storage/emulated$"`) do set "SDCARD_AVAIL=%%B"
