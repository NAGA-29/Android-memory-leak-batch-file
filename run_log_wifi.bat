...
for /f "usebackq delims=" %%A in (`adb -s %DEVICE% shell getprop ro.product.cpu.abi 2^>NUL`) do echo abi=%%A>>"%RUN_INFO_FILE%"
for /f "usebackq tokens=2" %%A in (`adb -s %DEVICE% shell cat /proc/meminfo 2^>NUL ^| findstr /B /C:"MemTotal"`) do echo mem_total_kb=%%A>>"%RUN_INFO_FILE%"
for /f "usebackq delims=" %%A in (`adb -s %DEVICE% shell getprop ro.build.version.release 2^>NUL`) do echo android_ver=%%A>>"%RUN_INFO_FILE%"
for /f "usebackq delims=" %%A in (`adb -s %DEVICE% shell dumpsys package %PACKAGE% 2^>NUL ^| findstr /R "versionName"`) do echo app_version=%%A>>"%RUN_INFO_FILE%"
for /f "usebackq delims=" %%A in (`adb -s %DEVICE% shell settings get global bluetooth_on 2^>NUL`) do echo bt=%%A>>"%RUN_INFO_FILE%"
...
echo iso_time,pid,uptime_s,pss_kb,native_heap_kb,dalvik_heap_kb,graphics_kb,cpu_pct,proc_name,memfree_kb,memavail_kb,cached_kb,swap_total_kb,swapfree_kb,swap_kb,swappss_kb,data_avail_kb,sdcard_avail_kb,pgfault,pgmajfault,pid_changed>"%METRICS_FILE%"
...
:loop
    ...
    for /f "usebackq tokens=2" %%A in (`adb -s %DEVICE% shell cat /proc/meminfo 2^>NUL ^| findstr /B /C:"SwapTotal"`) do set "SWAPTOTAL=%%A"
    for /f "usebackq tokens=2" %%A in (`adb -s %DEVICE% shell cat /proc/meminfo 2^>NUL ^| findstr /B /C:"SwapFree"`) do set "SWAPFREE=%%A"

    for /f "usebackq tokens=2,4" %%A in (`adb -s %DEVICE% shell df -k /data 2^>NUL ^| findstr /R /C:" /data$"`) do set "DATA_AVAIL=%%B"
    for /f "usebackq tokens=2,4" %%A in (`adb -s %DEVICE% shell df -k /storage/emulated 2^>NUL ^| findstr /R /C:" /storage/emulated$"`) do set "SDCARD_AVAIL=%%B"
    ...
    echo !CUR_ISO!,!PID!,!UPTIME!,!PSS!,!NATIVE!,!DALVIK!,!GRAPHICS!,!CPU!,%PACKAGE%,!MEMFREE!,!MEMAVAIL!,!CACHED!,!SWAPTOTAL!,!SWAPFREE!,!SWAP_PROC!,!SWAPPSS_PROC!,!DATA_AVAIL!,!SDCARD_AVAIL!,!PGFAULT!,!PGMAJFAULT!,!PID_CHANGE!>>"%METRICS_FILE%"
